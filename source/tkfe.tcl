...
MJrpNmVJEWDBDV6+sIR5ZeUWmCsxIgI40LGqSasmsLS4nfIhAAQgnD6BcvuEQwAJQiQxyhBgwhHFjAJUSAKZ0QUmlRkFAgA7
  ...
  image create photo icondirdenied_OLD -data {
...
#Окно не может перекрываться (yes)
    ...
    eval "$fm.dirs.t heading {#0} -text {[mc {Folders}]} -image downArrow -command {[namespace current]::columnSort $fm.dirs.t {#0} 1} "
    ...
    eval "$fm.files.t heading {#0} -text {[mc {Folders and files}]} -image upArrow -command {[namespace current]::columnSort $fm.files.t {#0} 0} "
    if {$typefb == "dir"} {
	$fm.files.t heading "#0" -text  [mc {Folders}]
    }

    if {$typefb != "dir"} {
      eval "$fm.files.t heading size -text {[mc {Size}]} -image upArrow -command {[namespace current]::columnSort $fm.files.t size 0}"
      eval "$fm.files.t heading date -text {[mc {Date}]} -image upArrow -command {[namespace current]::columnSort $fm.files.t date 0}"
      eval "$fm.files.t heading permissions -text {[mc {Permissions}]} -image upArrow -command {[namespace current]::columnSort $fm.files.t permissions 0}"
...
#		-command [list ::ttk::dialog::file::NewDirCmd $win]
    ...
    eval "$data(hiddenBtn) configure -image eye_hidden -padding {1mm 0 0 0} -variable FE::folder(hiddencb) -command {[namespace current]::gohidden $fm $typew $typefb $otv}"
...
#Состав расширенного просмотра
	...
	$data(lang) configure -image  ru_24x16
    } else {
	$data(lang) configure -image usa_24x16
    }
...
#puts "goreverse fm=$fm typefb=$typefb ::FE::folder(reverse)=$::FE::folder(reverse)"
  ...
  proc gohidden {fm typew typefb otv} {
	...
	$FE::folder(hiddenBtn) configure -image eye_hidden
    }
  }
...
#    set id [$tree insert $node end -id 0 -image icondirup -text ".." -values [list $levelup $type]]
#    $tree item $id -text ".."
#    set ind 1
    set ind 0
    if {$FE::folder(sepfolders)} {
    ...
    foreach f [lsort -dictionary $directory_list] {
      ...
      if {$rr == 0} {
        set id [$wtree insert $node end -id $ind -image icondirdenied -text [file tail "$f"] \
        -values [list "$f" "denied"]]
      } else {
        set id [$wtree insert $node end -id $ind -image icondir -text [file tail "$f"] \
        -values [list "$f" $type]]
      }
      incr ind
    }
